    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-white px-6">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900">Invalid Report Link</h1>
          <p className="mt-2 text-sm text-gray-500">
            This link is invalid or has expired.
          </p>
          <Link
            href="/analyze"
            className="mt-6 inline-flex items-center gap-2 rounded-full bg-indigo-600 px-6 py-3 text-sm font-semibold text-white shadow-lg transition-all hover:shadow-xl"
          >
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-200 bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-2.5">
            ...
            <span className="text-lg font-bold tracking-tight text-gray-900">DEALWISE</span>
          </Link>
      ...
      <main className="mx-auto max-w-4xl px-6 py-10">
        <h1 className="mb-8 text-center text-3xl font-bold text-gray-900">Contract Analysis Report</h1>

        {/* Score + Rate + Recommendation */}
        <div className="mb-8 grid gap-6 sm:grid-cols-3">
          {/* Score */}
          <div className="flex flex-col items-center justify-center rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-4 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            ...
            <div className="rounded-xl border border-gray-200 px-4 py-3">
              <p className="text-xs text-gray-400">Quoted Rate</p>
              <p className="text-xl font-bold text-gray-900">
                {data.cur}{data.nr.toFixed(2)}<span className="text-sm font-normal text-gray-400">/hr</span>
              </p>
            </div>
        ...
        <div className="mb-8 grid grid-cols-3 gap-4">
          <div className="flex flex-col items-center rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <span className="text-2xl font-bold text-gray-900">{data.rf}</span>
            <span className="text-xs text-gray-500">Red Flags</span>
          </div>
          <div className="flex flex-col items-center rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <AlertTriangle className="mb-2 h-5 w-5 text-amber-500" />
            <span className="text-2xl font-bold text-gray-900">{data.mc}</span>
            <span className="text-xs text-gray-500">Missing Clauses</span>
          </div>
          <div className="flex flex-col items-center rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <Shield className="mb-2 h-5 w-5 text-emerald-500" />
            <span className="text-2xl font-bold text-gray-900">{data.gf}</span>
            <span className="text-xs text-gray-500">Green Flags</span>
          </div>
        </div>

        {/* Summary */}
        <div className="mb-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <h3 className="mb-3 text-sm font-semibold uppercase tracking-wider text-gray-400">Summary</h3>
          <p className="text-sm leading-relaxed text-gray-600">{data.sum}</p>
        </div>

        {/* Top Red Flags */}
        {data.top && data.top.length > 0 && (
          <div className="mb-8 rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
            <h3 className="mb-4 text-sm font-semibold uppercase tracking-wider text-gray-400">Top Red Flags</h3>
            <div className="space-y-3">
              {data.top.map((flag, i) => {
                const sev = SEVERITY_STYLES[flag.sev?.toLowerCase()] ?? SEVERITY_STYLES.medium;
                return (
                  <div key={i} className="flex items-start gap-3 rounded-xl border border-gray-200 bg-white p-4">
        ...
        <div className="rounded-2xl border border-indigo-200 bg-indigo-50 p-8 text-center">
          <h3 className="mb-2 text-xl font-bold text-gray-900">Analyze Your Own Contract</h3>
          <p className="mb-6 text-sm text-gray-500">
            Know your real rate before you sign. Free, instant, and private.
          </p>
          <Link
            href="/analyze"
            className="inline-flex items-center gap-2 rounded-full bg-indigo-600 px-8 py-3.5 text-sm font-semibold text-white shadow-lg shadow-indigo-500/20 transition-all hover:shadow-xl hover:brightness-105"
          >
      ...
      <footer className="border-t border-gray-200 bg-white">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-6">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-gray-900">DEALWISE</span>
          </div>
          <p className="text-xs text-gray-400">Know your real rate</p>
        </div>
      </footer>
    </div>
  );
